    ...
    return (
        <div className='mt-[10vw] mb-[5vw] mx-[3vw]'>
            ...
            <div className='flex justify-center gap-1'>
                {
                    events.map((e,index) =>
                        <div key={e.name} className={`relative h-[40vw] ${index == focus ? ' w-[40vw]' : ' w-[10vw]'} font-josefin-sans transition-width duration-500 ease-in-out`}>
                            ...
                            <div  className={`absolute inset-0 bg-black ${index == focus ? 'bg-opacity-55' : 'bg-opacity-0'}   p-3 mt-[25vw] border-t-[4px] transition-width duration-500 ease-in-out`}>
                                <p className={`h-[10vh] text-orange-300 ${index == focus ? ' w-[25vw] text-5xl' : 'hidden text-2xl w-[8vw]'}`}>{e.name}</p>
                                <div className='flex items-center gap-2'>
                                    <FaLocationDot className={`text-white relative bottom-[2px] ${index == focus ? 'flex' : 'hidden '}`}/>
                                    <p className={`text-gray-200 text-xl ${index == focus ? 'flex' : 'hidden'}`}>{e.location}</p>
                                </div>
                                <div className='flex items-center gap-2'>
                                    <MdOutlineAccessTimeFilled className={`text-white relative bottom-[2px] ${index == focus ? 'flex' : 'hidden '}`}/>
                                    <p className={`text-gray-200 text-lg ${index == focus ? 'flex' : 'hidden'}`}>{e.date}</p>
                                </div>
                                ...
                                <div className={`mt-5 ml-2 w-[7vw] bg-orange-200 rounded-sm ${index == focus ? 'flex items-center' : 'hidden'}`}>
                                    <FaLock className={`text-black ${e.access == 'Private' ? "flex": "hidden" } `} />
                                    <MdOutlinePublic className={`text--black ${e.access == 'Public' ? "flex text-xl": "hidden" } `} />
                                    <p className='relative top-1 text-black bg-orange-200 rounded-r-sm rounded-b-xl px-1'>{e.access} Event</p>
                                </div>
